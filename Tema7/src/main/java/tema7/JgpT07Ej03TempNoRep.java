/*
Ejercicio 3 - Realiza un programa que tenga definido un Array como variable global llamado
...
a. Llenar el Array con valores al azar para todos los meses (entre 0 y 40) __NO REPES__.
...
public class JgpT07Ej03TempNoRep {
    ...
    // Método para llenar el array con valores al azar entre 0 y 40 ===NO REPES===
    public static void llenarArray() {
        Random rand = new Random(); 
        HashSet<Integer> temperaturas = new HashSet<>(); 
        int i = 0; 
        while (i < temperaturaMeses.length) { int temp = rand.nextInt(41); // Generar un número entre 0 y 40 
        if (!temperaturas.contains(temp)) { 
            temperaturaMeses[i] = temp; temperaturas.add(temp); 
            i++; 
        } 
        }
    }
